...
api: {
     apiKey:'2b6146dce6d353677d3c3e090f16beb4',
     baseUrl: 'https://api.themoviedb.org/3/'
}
}

//Display show details
displayShowDetails = async() => {
...
const searchAPIData = async () => {
    ...
    const BASE_URL = global.api.baseUrl;
    showSpinner();
    const response = await fetch(`${BASE_URL}search/${global.search.type}?api_key=${API_KEY}&language=en-US&query=${global.search.term}&page=${global.search.page}`);
    const data = await response.json();
    hideSpinner();
    return data;
    }
...
const search = async() => {
    ...
    if(results.length === 0){
        showAlerts( 'No results found');
    return;
    }
    displaySearchResults(results);
    document.querySelector('#search-term').value = '';
    }else{
        showAlerts('Please enter a search term');
    }
...
const displaySearchResults = (results) => {
    ...
    results.forEach((result) => {
        ...
        ${ result.poster_path ? `<img src="https://image.tmdb.org/t/p/w500/${result.poster_path}" class="card-img-top" alt="${global.search.type === 'movie'? result.title: result.name}">` : `<img
        src="../images/no-image.jpg"
        class="card-img-top"
...
//Show alerts
showAlerts = (message, className='error') => {
